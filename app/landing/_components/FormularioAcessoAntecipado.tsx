...
function inputStyle(comErro: boolean): React.CSSProperties { return { width: '100%', minHeight: 44, padding: '11px 13px', borderRadius: 9, border: `1px solid ${comErro ? '#e0526b' : '#b9cbdc'}`, background: '#ffffff', color: '#10263d', fontSize: 14, outline: 'none', colorScheme: 'light' } }

const S: Record<string, React.CSSProperties> = {
  section: { padding: '56px 0', background: 'linear-gradient(180deg,#edf5fa,#f7fafc)', borderTop: '1px solid #dce7ef', borderBottom: '1px solid #dce7ef' }, wrap: { width: 'min(980px, calc(100% - 40px))', margin: '0 auto' }, heading: { maxWidth: 760, marginBottom: 24 }, eyebrow: { display: 'inline-flex', color: '#087fa3', border: '1px solid rgba(16,169,209,.34)', background: 'rgba(16,169,209,.08)', padding: '6px 10px', borderRadius: 99, fontSize: 11, fontWeight: 700, letterSpacing: '.055em', textTransform: 'uppercase' }, title: { margin: '14px 0 10px', fontFamily: '"Segoe UI Variable Display", "Segoe UI", Arial, sans-serif', fontWeight: 500, fontSize: 32, lineHeight: 1.18, letterSpacing: '-.025em', color: '#10263d' }, lead: { margin: 0, color: '#526a82', fontSize: 16, lineHeight: 1.65 }, form: { position: 'relative', padding: 24, borderRadius: 18, border: '1px solid #d6e1ec', background: '#ffffff', boxShadow: '0 18px 50px rgba(25,59,91,.09)' }, grid2: { display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 16 }, label: { display: 'block', marginBottom: 6, color: '#334d66', fontSize: 12.5, fontWeight: 650 }, fieldset: { margin: '22px 0 0', padding: 0, border: 0 }, legend: { color: '#10263d', fontSize: 15, fontWeight: 650 }, help: { margin: '4px 0 12px', color: '#6b8095', fontSize: 12.5 }, purposeGrid: { display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 10 }, purpose: { display: 'flex', alignItems: 'center', gap: 10, padding: 12, borderRadius: 11, border: '1px solid #d6e1ec', background: '#f9fbfd', cursor: 'pointer', color: '#10263d', fontSize: 13.5 }, purposeActive: { borderColor: '#10a9d1', background: 'rgba(16,169,209,.08)' }, consents: { display: 'grid', gap: 11, marginTop: 22 }, consentLabel: { display: 'flex', alignItems: 'flex-start', gap: 10, color: '#526a82', fontSize: 12.5, lineHeight: 1.55, cursor: 'pointer' }, link: { color: '#087fa3', fontWeight: 650 }, error: { display: 'flex', alignItems: 'center', gap: 5, margin: '7px 0 0', color: '#d73f59', fontSize: 12 }, submit: { width: '100%', marginTop: 22, minHeight: 46, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, border: 0, borderRadius: 10, background: '#10a9d1', color: '#052235', fontSize: 14, fontWeight: 650, cursor: 'pointer', boxShadow: '0 8px 20px rgba(16,169,209,.18)' }, success: { display: 'grid', gridTemplateColumns: 'auto 1fr', alignItems: 'center', gap: 16, padding: 24, borderRadius: 18, border: '1px solid rgba(22,138,85,.32)', background: '#edf9f3' }, honeypot: { position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' },
}
